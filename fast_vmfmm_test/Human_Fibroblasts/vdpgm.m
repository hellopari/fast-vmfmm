...
if strcmp(opts.algorithm,'vdp')
   ...
   if(nargin==3)
    extra_V.kk = g;
        ...
        extra_V.kk =[112,114];
%         [240,250];[112,114]  0.4931;[100,102]0.4736;
    end
...
while 1
  ...
  if isequal(opts.algorithm, 'vdp') & sum(r(:,end)) >0.01
    r(:,end+1) = 0;
    extra_V.kk_approx(:,end+1) = 250;
    extra_V.kk(:,end+1)=250;
    extra_V.ln_kk(:,end+1)=0.6; 
  end
  ...
  if isequal(opts.algorithm, 'vdp') & sum(r(:,end-1)) <0.0001
      r(:,end-1) = [];
...
% partition.sigma = squareform(1-pdist(data_a,'cosine')) + eye(size(data_a,1));
partition.children = [];
...
hp_prior.b=0.3; % but do not trust that too much
