...
elseif strcmp(op.model_Type,'VMM') 
    ...
    nu=D/2-1;
    const_kk = prior.a.*log(prior.b)-gammaln(prior.a);
    E_m_H_m = (nu+1/2)*log(prior.beta./post.beta) + post.beta - prior.beta + prior.beta.*extra.kk.*sum(m_o.*post.mu) - post.beta.*extra.kk;        
    E_k_H_k = const_kk +(prior.a-1).*extra.ln_kk-prior.b.*extra.kk+post.a-log(post.b)-gammaln(post.a)+(1-post.a).*psi(post.a);
    term4 = sum(E_m_H_m)+sum(E_k_H_k);
end
